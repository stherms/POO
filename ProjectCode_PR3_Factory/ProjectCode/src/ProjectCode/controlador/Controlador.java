...
public class Controlador {
    ...
    public void CrearSocioFederado(int numSocio, String nombre, String nif, String codigoFederacion, String nomFederacion){
        try {
            //Crear un objeto socio Factory
            SocioFactory socioFactory = new SocioFactory();
            //Llamar a getSocio del socioFactoy pasandole todos los parametros, los que no use, poner cualquier valor
            B0_Socio socioFederado = socioFactory.getSocio("FEDERADO", numSocio, nombre, nif, null, 0, codigoFederacion, nomFederacion);
            socioFederadoDAO.crearSocioFederado((B2_SocioFederado) socioFederado);
        }
    ...
    public ArrayList<B2_SocioFederado> mostrarSocioFederados(){
        ...
        try {
            socioFederados = socioFederadoDAO.mostrarSocioFederado();
        }
        catch (SQLException ex){
            ex.printStackTrace();
        }
    ...
    public void CrearSocioEstandar(int numSocio, String nombre, String nif, C0_Seguro seguro){
        try {
            //Utilizando un objeto con el constructor
            //B1_SocioEstandar socioEstandar = new B1_SocioEstandar(numSocio, nombre, nif, seguro);

            //Crear un objeto socio Factory
            SocioFactory socioFactory = new SocioFactory();
            //Llamar a getSocio del socioFactoy pasandole todos los parametros, los que no use, poner cualquier valor
            B0_Socio socioEstandar = socioFactory.getSocio("ESTANDAR", numSocio, nombre, nif, seguro, 0, null, null);
            socioEstandarDAO.crearSocioEstandar((B1_SocioEstandar) socioEstandar);
        }
        catch (SQLException ex){
            ex.printStackTrace();
        }
    ...
    public ArrayList<B1_SocioEstandar> mostrarSocioEstandar(){
        ...
        try {
            socioEstandars = socioEstandarDAO.mostrarSocioEstandar();
        }
        catch (SQLException ex){
            ex.printStackTrace();
        }
    ...
    public void CrearSocioInfantil(int numSocio, String nombre, int numSocioPadre){
        ...
        try {
            SocioFactory socioFactory = new SocioFactory();
            //Llamar a getSocio del socioFactoy pasandole todos los parametros, los que no use, poner cualquier valor
            B0_Socio socioInfantil = socioFactory.getSocio("INFANTIL", numSocio, nombre, null, null, numSocioPadre, null, null);
            socioInfantilDAO.crearSocioInfantil((B3_SocioInfantil) socioInfantil);
        }
        catch (Exception ex){
            ex.printStackTrace();
        }
    ...
    public ArrayList<B3_SocioInfantil> mostrarSocioInfantil(){
        ...
        try {
            socioInfantil = socioInfantilDAO.mostrarSocioInfantil();
        }
        catch (SQLException ex){
            ex.printStackTrace();
        }
